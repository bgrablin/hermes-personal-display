(() => {
  ...
  const renderer = new window.HermesCharacterRgsRenderer('display-root', PRESETS.idle_watchful);
  ...
  document.querySelectorAll('[data-preset]').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      writePacket(preset, `Previewing RGS modular state: ${button.dataset.preset}.`);
    });
  });
  ...
  document.querySelector('#randomize').addEventListener('click', () => {
    ...
    currentPacket = window.HermesDisplayState.normalizePersonaPacket(nudged);
    textarea.value = JSON.stringify(currentPacket, null, 2);
    skinSelect.value = currentPacket.skin;
    renderer.setPacket(currentPacket);
    // Trigger the visible character nudge after setPacket so its gesture wins over the
    // state-flash gesture queued by setPacket. The renderer picks a nudge variant
    // appropriate to the current mood (peek/perk/shuffle/squint/sleep_drift).
    const nudge = renderer.triggerNudge();
    output.value = `Random nudge: ${nudge.label} (${Math.round(nudge.duration)}ms) — drives + quip refreshed.`;
  });
  ...
  function chooseQuip(packet) {
    const quips = {
      idle_watchful: ['Quiet systems. Suspicious.', 'Standing by with opinions.', 'Watching the home base.'],
  ...
  writePacket(currentPacket, 'Ready. RGS modular familiar loaded. Use buttons or keys 1-5.');
})();
