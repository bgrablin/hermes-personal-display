(() => {
  ...
  const renderer = new window.HermesFoozleRenderer('display-root', PRESETS.idle_watchful);
  ...
  document.querySelectorAll('[data-preset]').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      writePacket(preset, `Previewing Foozle state: ${button.dataset.preset}.`);
    });
  });

  skinSelect.addEventListener('change', () => {
    writePacket(selectedSkinPacket(currentPacket), `Skin metadata switched to ${skinSelect.value}; Foozle sprite source remains unchanged.`);
  });
  ...
  document.querySelector('#randomize').addEventListener('click', () => {
    ...
    writePacket(nudged, 'Randomized runtime drives and quip. Foozle state remains finite.');
  });
  ...
  function chooseQuip(packet) {
    const quips = {
      idle_watchful: ['Standing by. Mostly.', 'Quiet systems. Suspicious.', 'Watching the home base.'],
      thinking_focused: ['Indexing the obvious.', 'Poking the machinery.', 'Logs are judging us.'],
      healthy_smug: ['All green. Suspiciously tidy.', 'No fires. Tiny miracle.', 'Uptime tastes like victory.'],
      blocked_annoyed: ['Blocked. Naturally.', 'Waiting on a human API.', 'Approval wall detected.'],
      night_sleepy: ['Low glow. Still watching.', 'Dim, not gone.', 'Guard mode: sleepy edition.']
    };
  ...
  writePacket(currentPacket, 'Ready. Foozle robot route loaded. Use buttons or keys 1-5.');
})();
