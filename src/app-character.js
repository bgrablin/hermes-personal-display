(() => {
  ...
  const renderer = new window.HermesCharacterSvgRenderer('display-root', PRESETS.idle_watchful);
  ...
  document.querySelectorAll('[data-preset]').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      writePacket(preset, `Previewing state-machine preset: ${button.dataset.preset}.`);
    });
  });
  ...
  document.querySelector('#randomize').addEventListener('click', () => {
    ...
    writePacket(nudged, 'Randomized runtime drives and quip. State graph remains finite.');
  });
  ...
  writePacket(currentPacket, 'Ready. Character runtime route loaded. Use buttons or keys 1-5.');
})();
