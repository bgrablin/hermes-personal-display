...
Observer version 1.3 also retains a bounded, turn-scoped tool-call rail keyed only by Hermes' exact `tool_call_id`. It stores tool name, call/turn identity, status, duration and observation timestamps—not arguments or output. Parallel calls settle independently. A finished turn with no matching terminal hook changes that call to `unknown`; it never guesses success from parent completion. Because `on_session_start` is session-scoped, the first event carrying a different `turn_id` rotates the current rail and clears prior-turn interruption metadata; unresolved prior tool evidence remains a fail-closed outcome. Unknown rows cannot be evicted as settled capacity. The operator panel shows at most eight cards at once while the snapshot retains at most 64 calls.

This relies on Hermes main's call-identity hook gate: [`4121aa29`](https://github.com/NousResearch/hermes-agent/commit/4121aa295a6d28f8acddb5fd68026c6e6629e22e), with timed-out-worker and race corrections in [`cdd58810`](https://github.com/NousResearch/hermes-agent/commit/cdd58810a406d416a8656f404f9c893a7c72ddd1), [`4bd38ec9`](https://github.com/NousResearch/hermes-agent/commit/4bd38ec9fdb2754dee3b12cab5a63a17c2015f9f) and [`73f808e4`](https://github.com/NousResearch/hermes-agent/commit/73f808e47f9d331f71389c1a3e306f351b735be6). Issue [#98382](https://github.com/NousResearch/hermes-agent/issues/98382) is closed as completed. Older Hermes revisions can still drop healthy overlaps before the observer sees them, so upgrade Hermes before deploying observer 1.3. Genuine callback timeouts remain best-effort and are not a replay protocol.

The display collector reads these snapshots without importing Hermes or launching an agent. Positive observed work drives the existing resolver and eye/headline contract. A background command survives parent completion or interruption. Missing registry entities, stale process epochs, and event loss produce unknown rather than idle or successful completion. Sources older than 20 seconds are stale. The latest terminal observation is presented briefly. An explicit user interruption is amber; a structured Hermes watchdog/lifecycle interruption is coral and requests attention without becoming a false tool/provider failure. An explicit failed/error/stalled state or nonzero process exit still gets failure attention. A process exit code is separate from the parent outcome.
...
Hermes now requires WebSocket clients that answer server-to-client requests to advertise `client.capabilities {server_requests: true}` after `gateway.ready`. The capability covers the entire request channel, including vault and setup prompts, not just approvals and clarifications. That is intentionally **not** the display monitor's role: it never attaches to a session, renders those requests, or claims it can answer them. Do not add the advertisement without also adding the complete owner-bound request lifecycle. The passive reader may continue its exact-owner status/control reads without becoming a prompt surface.
...
RPC snapshots now supply server-relative `age_seconds`, avoiding comparison of browser and host wall clocks. The browser conservatively includes request elapsed time and uses monotonic/wall elapsed time to age held data. Older servers without this age field retain readable details but cannot enable browser controls. This supplements, not replaces, server-side owner/revision checks. The underlying observer remains best effort: the healthy-overlap defect is fixed on current Hermes main, but genuine callback timeouts, process loss and queue overflow are not replayable evidence.
...
Hermes main now fires `agent_loop_stopped` immediately for an active messaging-gateway `/stop` or running-agent `/new`, and for TUI/Desktop `session.interrupt`. The observer resolves the supplied `session_key` to an exact alias previously observed inside the same profile; it never guesses between concurrent sessions. If no alias is available, the exact transport key remains a separate observed row rather than being attached to an arbitrary parent. Only the bounded platform and interruption reasons are retained. It does not infer an interrupt from prose or a disconnected transport.

The parent turn changes to `interrupted` immediately. The ambient display reports a recent interruption instead of painting an error, and private touch inspection adds an amber **TURN INTERRUPTED** card. Opening an interrupted session brings that outcome card into the visible panel without requiring a blind landscape swipe; the profile identity and summary remain immediately above it. Processes, delegation units and subagents keep their own states, so an interrupted parent can still truthfully say that background work continues. A reduced later `on_session_end` cannot erase the observed interruption; an explicit hard-failure finalizer still wins. A new turn clears the old interruption detail. Explicit failed/error/stalled process rows remain hard failures even when an exit code is absent. No stop button, attach, retry, replay or new authority is added. [Synthetic preview](interrupted-turn-observation-2026-09-14.svg).

Observer 1.3 additionally accepts Hermes' bounded canonical exit identities: `interrupted_by_user`, `interrupted_during_api_call`, `interrupted_by_system(<issuer>)`, and `interrupted_during_api_call(<issuer>)`. Only a safe 64-character issuer slug crosses the callback boundary; unrecognized raw exit text is discarded before queueing. System stops say **Stopped by Hermes**, use coral attention styling, and may show the safe issuer, provider-request phase, and observed surface. They do not overwrite or settle independently tracked background work. [System-attribution preview](system-interruption-attribution-2026-09-17.svg).
...
System attribution is newer again: it requires Hermes [`9d0ab880`](https://github.com/NousResearch/hermes-agent/commit/9d0ab880bc5c70c904779bf3fd1c21abd9232258) or later. Release `v2026.9.14` alone does not include that contract. Keep observer 1.3 undeployed until Silver runs that commit or later and host-side Plugin Doctor passes.
...
An amber **Outcome needs verification** card means Hermes lost an MCP stdio transport after dispatch and deliberately did not replay the possibly completed side effect. This state is neither failure nor success. The display retains the useful tool name and bounded, credential-redacted diagnostic, keeps the visible activity result unknown, and offers no retry action. Verify the external system before invoking the tool again.
