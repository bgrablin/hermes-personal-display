...
Expected accepted runtime: CPU package temperature generally below 80°C under the current kiosk load, with prior isolation testing near 70°C, CPU max performance capped at 80%, DP-2 still `1920x1280` inverted, and no visible regression in motion or visual direction. Ambient temperature and workload can change the observed package temperature.
